...
最初に筆者の立場を書いておきます。筆者はHubSpot Japanでカスタマーサクセスマネージャー（CSM）として約4年間、200社以上の活用支援に携わってきました。Salesforceについては、ユーザーとして3年以上の利用経験があり、認定資格を5つ保有しています。Zohoについては、今回の記事のためにトライアル環境を実際に構築・操作し、公開情報とあわせて検証しました。長期の運用支援経験にもとづくものではない点を、先にお断りしておきます。
...
- **Zoho**：これは見た目の印象より深い、というのが検証してみての率直な感想です。カスタムタブ（カスタムオブジェクトに相当）、商談のステージごとに入力必須項目を強制する仕組み、承認フロー、さらには独自のスクリプト言語まで揃っています。一方で、検証中に感じたのは「深さはあるのに、画面の分かりづらさゆえに機能を発見しにくい」という点でした。
...
- **Zoho**：実際に操作してみると、選択肢が多く、直感的とは言いづらいと感じました。設定画面もアプリごとに分かれており、「あの設定はどこにあったか」を探すのに時間がかかる場面が何度かありました。慣れるまでの学習コストは高いと感じます。海外のユーザーコミュニティでも同様の声は見られます。
...
- **Zoho**：CRM内のワークフローに加え、マーケティング側の多段階のシナリオ（例：メール開封の有無で次のアクションを分岐させる）は、別アプリのオートメーション機能で組む構造になっています。
...
- **Zoho**：「レポート」と「アナリティクス」の2つに機能が分かれており、検証時には、目的のレポートを作成するのに結構苦労した印象があります。
...
- **Salesforce**：一見連携しているように見えますが、実はデータベースが分離されており、あくまで「連携させている」という仕様です。Salesforce上に表示することは可能ですが、セールス側の行動履歴（ミーティングやコールなど）と同じ軸では載ってこないため、かなり分かりづらい。筆者が支援してきた中にも、これを理由にPardotの利用をやめたお客様がいました。
- **Zoho**：CRMとMA（Zoho Marketing Automation）は別データベースで、ネイティブ同期でつなぐ構造です。構造としてはSalesforce＋Pardotに近いと考えてよいでしょう。CRM側からはリード・コンタクト・カスタムモジュールのいずれか1つを選んで同期し（一度選ぶと後から変更できません）、MA側からCRMへ戻せるのは、公式ドキュメント上はオプトアウト状況・コンタクトスコア・コンタクトステージの3つと、メールキャンペーンの結果です。

  注意したいのは、**Webサイトの閲覧履歴がそのままCRMに流れるわけではない**点です。MA側でトラッキングした行動は、スコアやステージという加工済みの値としてCRMに反映されます。営業がCRM上で「この人がどのページを見たか」を見たい場合は、さらに別アプリ（Zoho SalesIQ）の導入が必要になり、CRM＋MA＋SalesIQという3アプリ構成になります。

  連携そのものは想像以上に作り込まれています。ただし、どのデータをどちら向きに流すかを自分で設計する必要があり、「契約すれば自動的につながる」わけではありません。これがHubSpotの単一データベース構造との最大の違いです。
...
- **Salesforce**：社内でAIエージェント（自律的に業務をこなすAI）を構築できる「Agentforce」を持ち、大企業のサポートチャットなどでの導入実績があります。長らく構築難易度の高さが課題で、筆者自身かなり勉強しましたが、正直に言って難しいと感じました。ただし2026年に入って状況は動いています。公式発表によれば、Summer '26でエージェント用のスクリプト言語「Agent Script」と新しいAgentforce Builderが正式リリースされ、既存エージェントのワンクリック移行にも対応しました。

  さらに大きいのが、2026年4月に正式リリース（GA）されたSalesforceホステッド型MCPサーバーです。公式ドキュメントによれば、Enterprise Edition以上の組織であれば、ClaudeやChatGPTといったAIクライアントからSalesforceのデータ・フロー・Apexアクションを操作できます。既存の権限設定（CRUD・項目レベルセキュリティ・共有ルール）がそのまま適用され、処理は認証ユーザーの権限で実行されて監査ログに残る設計とされています。「AIに任せると誰が何をしたか分からなくなる」という懸念に、Salesforceらしい統制の効かせ方で答えた形です。なお、この点は公式情報にもとづく記述であり、筆者自身の実機検証はこれからです。

  一方で、Enterprise Edition以上が前提であり、Agentforceを本格運用するにはData Cloudを含めた土台の整備が必要です。手軽に始められる性質のものではない、という点は変わっていません。
...
| MA連携 | △ データベースが分離 | ◎ 単一DBで一元管理 | △ 別DBを同期。Web行動は別アプリ |
| AI連携・AI対応 | ◎ MCPがGA。統制は最も堅い（EE以上） | ◎ Breeze＋外部AI連携が優秀 | △ 実機未検証（API連携は可能） |
...
なお、すでにSalesforceを使っていて乗り換えを検討している場合は、[SalesforceからHubSpotへの移行](/blog/26-salesforce-hubspot-migration/)の費用・期間・進め方をまとめた記事もあわせてご覧ください。
